...
(function (HtmlSubjectType) {
  HtmlSubjectType["input"] = "input";
  HtmlSubjectType["dragAndDrop"] = "drag-n-drop";
})(HtmlSubjectType || (HtmlSubjectType = {}));
...
function attachFileToElement(subject, {
  ...
  cy.stub(dataTransfer, 'items').callsFake(() => Array.from(dataTransfer.items).map(item => {
  ...
  if (subjectType === HtmlSubjectType.input) {
    ...
    const inputElement = inputElements.length === 1 ? inputElements[0] : subject[0];
    inputElement.files = dataTransfer.files;
...
(function (FileEncodings) {
  FileEncodings["ascii"] = "ascii";
  FileEncodings["base64"] = "base64";
  FileEncodings["binary"] = "binary";
  FileEncodings["hex"] = "hex";
  FileEncodings["latin1"] = "latin1";
  FileEncodings["utf8"] = "utf8";
  FileEncodings["utf-8"] = "utf-8";
  FileEncodings["ucs2"] = "ucs2";
  FileEncodings["ucs-2"] = "ucs-2";
  FileEncodings["utf16le"] = "utf16le";
  FileEncodings["utf-16le"] = "utf-16le";
...
const ENCODING_TO_FILE_CONTENTS_GETTER = {
  'ascii': fileContent => fileContent,
  'base64': (fileContent, mimeType) => Cypress.Blob.base64StringToBlob(fileContent, mimeType),
  'binary': (fileContent, mimeType) => Cypress.Blob.binaryStringToBlob(fileContent, mimeType),
  'hex': fileContent => fileContent,
  'latin1': fileContent => fileContent,
  'utf8': fileContent => fileContent,
  'utf-8': fileContent => fileContent,
  'ucs2': fileContent => fileContent,
  'ucs-2': fileContent => fileContent,
  'utf16le': fileContent => fileContent,
  'utf-16le': fileContent => fileContent
};
function getFileBlob({
  ...
  let fileContents = getFileContents(fileContent, mimeType); // https://github.com/abramenal/cypress-file-upload/issues/175
  ...
  const file = new window.File([fileContents], fileName, {
    type: mimeType,
    lastModified
  });
  return file;
...
const isKnownExtension = extension => Object.values(FileExtension).includes(extension);
...
function resolveFile({
  ...
  const fileMimeType = mimeType != null ? mimeType : getFileMimeType(filePath);
  const fileEncoding = encoding != null ? encoding : getFileEncoding(filePath);
  const fileLastModified = lastModified != null ? lastModified : Date.now();
  return new Cypress.Promise(resolve => cy.readFile(filePath, fileEncoding).then(fileContent => getFileBlob({
    fileContent,
    fileName,
    mimeType: fileMimeType != null ? fileMimeType : undefined,
...
function resolveFixture({
  ...
  const fileEncoding = encoding != null ? encoding : getFileEncoding(filePath);
  const fileLastModified = lastModified != null ? lastModified : Date.now();
  return new Cypress.Promise(resolve => cy.fixture(filePath, fileEncoding).then(fileContent => getFileBlob({
    fileContent,
    fileName,
...
var validateOptions = (({
  ...
  if (Object.values(HtmlSubjectType).indexOf(String(subjectType)) === -1) {
...
function attachFixture(subject, fixtureOrFixtureArray, processingOptions) {
  const {
    subjectType,
    force,
    allowEmpty
...
const initialize = () => {
  ...
  Cypress.Commands.add('attachFixture', {
    prevSubject: true
  }, attachFixture);
};
